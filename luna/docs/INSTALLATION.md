...
## 2. Déposer les trois artefacts

Récupère le dépôt sur ton poste :

```bash
git clone https://github.com/Guillaume-Alard/Home-Assistant.git
cd Home-Assistant/luna
```

### Par Samba (glisser-déposer)

| Depuis le dépôt | Vers le partage |
|---|---|
| tout le dossier `addon/` | `addons/luna/` — **renomme-le `luna`** |
| `integration/custom_components/luna/` | `config/custom_components/luna/` |
| `card/luna-card.js` | `config/www/luna-card.js` |

Trois précisions qui font perdre du temps sinon :

- Le dossier doit s'appeler **`luna`** dans `/addons`, et contenir `config.yaml`
  **à sa racine** — pas `addons/addon/config.yaml`.
- **`config/www/` n'existe probablement pas.** Crée-le. C'est ce dossier que
  Home Assistant sert sous l'URL `/local/`.
- Tu peux laisser `tests/` : ça n'alourdit que l'image. Le script de déploiement
  les exclut, un glisser-déposer non.
...
Il fait les trois copies, exclut les tests et les caches, et rappelle la suite.
...
Le plus souvent, `config.yaml` n'est pas à la racine. Le chemin exact doit être
`/addons/luna/config.yaml`. Ensuite seulement, **⋮ → Vérifier les mises à jour**
(un simple rechargement de page ne suffit pas).
