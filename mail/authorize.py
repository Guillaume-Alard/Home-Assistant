#!/usr/bin/env python3
"""Autorisation Gmail (une seule fois) → jeton de rafraîchissement pour Sentinel.
...
(GMAIL_REFRESH_TOKEN). Portée demandée : gmail.readonly (lecture seule).

Aucune dépendance : uniquement la bibliothèque standard de Python 3.

    python mail/authorize.py --client-id XXX --client-secret YYY
    # (ou définis GMAIL_CLIENT_ID / GMAIL_CLIENT_SECRET dans l'environnement)

Prérequis (voir docs/EMAIL.md) : un projet Google Cloud avec l'API Gmail activée
et un identifiant OAuth de type « Application de bureau ».
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Autorisation Gmail lecture seule pour Sentinel")
