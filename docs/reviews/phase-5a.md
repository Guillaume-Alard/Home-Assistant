...
- Tests : 118 (6 nouveaux — détecteur unitaire, service injoignable, veille →
  détection → tour vocal de bout en bout, non configuré ; faux openWakeWord).
...
```bash
cd core && pip install -r requirements-dev.txt && pytest -q   # 118 tests
```
